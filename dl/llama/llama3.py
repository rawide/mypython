...
if len(sys.argv) != 6:
    print("please input the model parameters, ./macs.py [hidden_dims] [decoder layers] [in_lens] [out_lens] [BW(TB/s)]\n")
    exit()
...
def weight(len, wt_type='INT8', kvcache_type='FP16'):
    ...
    if kvcache_type == 'FP16':
        ...
        kvtype = 1
    W_Q = word_dims*word_dims + word_dims
    W_KV = word_dims*word_dims/group_num + word_dims/group_num
    W_QKV = W_Q + W_KV*2
    W_kvcache = (len*word_dims/group_num + word_dims/group_num)*2*kvtype # treat k,v as wt, read and write (4K/8)BW
    W_LINEAR = word_dims*word_dims + word_dims
    W_FFN = (word_dims*ffn_dims+ffn_dims)*2 + word_dims*ffn_dims + word_dims
    #W_NORM = word_dims*2*2
    W_DECODER = decoder_layers * ( dlen*(W_QKV + W_FFN + W_LINEAR)+ W_kvcache)
    #W_DECODER = decoder_layers * ( dlen*(W_QKV + W_FFN )+ W_kvcache)
    # print(W_DECODER/decoder_layers)
    W_OUT_LINEAR = vocab_size*word_dims + vocab_size
    return W_DECODER + W_OUT_LINEAR*dlen

def total_weight(in_len, out_len):
    sum = 0
...
print("under %.2fTB/s BW, ideal perf is %.2ftoken/s@FP8"%(bw, out_len*bw/(wt_size/2**40)))
